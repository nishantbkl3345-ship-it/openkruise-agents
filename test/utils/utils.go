...
func UninstallCertManager() {
	...
	cmd := exec.Command("kubectl", "delete", "-f", url) // #nosec G204 -- test utility with controlled args
	...
	for _, lease := range kubeSystemLeases {
		cmd = exec.Command("kubectl", "delete", "lease", lease, // #nosec G204 -- test utility with controlled args
...
func InstallCertManager() error {
	url := fmt.Sprintf(certmanagerURLTmpl, certmanagerVersion)
	cmd := exec.Command("kubectl", "apply", "-f", url) // #nosec G204 -- test utility with controlled args
	if _, err := Run(cmd); err != nil {
		return err
	}
...
func UncommentCode(filename, target, prefix string) error {
	// nolint:gosec
	// #nosec G304 -- false positive, test utility
	...
	// nolint:gosec
	// #nosec G306 -- false positive, test utility
